...
function setup() {
  ...
  for(let i=0; i< N_walker; ++i){
    ...
    walkers.push(new random_walker(2,0.2 ));
  }
}
      
class random_walker {
  constructor(r=1,dphi=0.2,dcol=1,phi0=-1,ddphi=0.1) {
    this.x = width/2;
    this.y = height/2;
    this.r = r;
    if (phi0 < 0)
  ...
  step() {
    ...
    this.old_x = this.x;
    this.old_y = this.y;
    this.dphi += - 0.01*(this.phi)*dt + 0.05*sqrt(dt)*random();
    this.phi += this.dphi;
