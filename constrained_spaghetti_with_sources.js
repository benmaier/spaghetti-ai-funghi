...
let N_sources = 50;
let frame = 0;
let max_frames_between_disruption = 100;
let next_disruption = 100;
let use_disruptions = false;
...
function setup() {
  ...
  for(let i=0; i< N_walker; ++i){
    ...
    walkers.push(new random_walker(7,0.2 ));
  }
...
class random_walker {
  constructor(r=1,dphi=0.2,dcol=1,phi0=-1,dstroke=0.05) {
    this.x = width;///2;
    this.y = height;//2;
    //this.x = width * random();
    ...
    this.stroke_weight_upper_bound = 1.5;
...
class source extends random_walker {
  constructor(R=50,force_strength=1000,r=.1,dphi=0.2,dcol=1,phi0=-1) {
  ...
  draw(col=240) {
    fill(this.color);
    strokeWeight(this.stroke_weight);
    ellipse(this.old_x, this.old_y, this.R, this.R);    
  }
...
function draw() {
  ...
  if (use_disruptions && (frame >= next_disruption)){
    sources.forEach(source => source.shuffle());
    console.log("hey");
    frame = 0;
    next_disruption = random()*max_frames_between_disruption;
  }
}
